...
# - Se mantiene la logica base de señal/entrada/salida
...
# - Se adopta como nueva base el bloqueo adicional por última operación perdedora entre 5 y 9 días
# ============================================================
...
from A_configuracion import (
    ...
    TRAILING_STOP_PCT,
    FRECUENCIA_REVISION_REGIMEN,
    PERIODO_SMA200_REGIMEN,
    VENTANA_RETORNO_63_REGIMEN,
    VENTANA_CRUCES_SMA50_REGIMEN,
    UMBRAL_CRUCES_SERRUCHO,
    REGIMEN_AGRESIVO,
    REGIMEN_DEFENSIVO,
    SIZING_AGRESIVO_PORCENTAJE_CAPITAL,
    SIZING_AGRESIVO_MAX_UNIDADES,
    SIZING_DEFENSIVO_PORCENTAJE_CAPITAL,
    SIZING_DEFENSIVO_MAX_UNIDADES,
)
...
def evaluar_regimen(variables_regimen: Dict[str, Any], qqq_close_referencia: float) -> Dict[str, Any]:
...
def preparar_datos(
    ...
    regimen_actual = REGIMEN_DEFENSIVO
    ...
    for fecha in fechas:
        ...
        sma_larga = _media_simple(closes, len(closes) - 1, PERIODO_MEDIA_LARGA)
        row["qqq_media_larga"] = sma_larga
        row["senal_base_on"] = bool(sma_larga is not None and close_float > sma_larga)
        ...
        if _es_momento_revision_regimen(fecha, ultima_semana_revisada):
            ...
            info_regimen = evaluar_regimen(variables_regimen, qqq_close_referencia=close_float)

            if modo_selector == "FORZAR_AGRESIVO":
                info_regimen["regimen"] = REGIMEN_AGRESIVO
                info_regimen["motivo_regimen"] = "forzado => AGRESIVO"
            elif modo_selector == "FORZAR_DEFENSIVO":
                info_regimen["regimen"] = REGIMEN_DEFENSIVO
                info_regimen["motivo_regimen"] = "forzado => DEFENSIVO"

            regimen_actual = info_regimen["regimen"]
            ultima_info_regimen = info_regimen
            ultima_semana_revisada = (fecha.isocalendar().year, fecha.isocalendar().week)

        row["regimen"] = regimen_actual
        ...
        diagnostico_regimen_tsv.append(
            {
                ...
                "sma200_referencia": None
                if op.get("sma200_referencia") is None
                else round(float(op["sma200_referencia"]), 6),
