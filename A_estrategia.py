...
def preparar_datos(df_qqq: pd.DataFrame, df_qqq3: pd.DataFrame, df_vix: pd.DataFrame) -> pd.DataFrame:
    ...
    - Señal en QQQ
    - Ejecución en QQQ3
    - Señal con cierre de hoy
    - Ejecución con open del día siguiente
    """
...
def ejecutar_estrategia(df: pd.DataFrame) -> pd.DataFrame:
    """
    Backtest diario más realista:
    - La señal se decide con el cierre del día actual
    - La ejecución se hace al open del día siguiente
    - Entrada: señal confirmada y sin posición
    - Salida: trailing o señal OFF
    """
    ...
    # Recorremos hasta penúltima fila porque ejecutamos en el open del siguiente día
    for i in range(len(df) - 1):
        fila_hoy = df.iloc[i]
        fila_manana = df.iloc[i + 1]

        fecha_hoy = fila_hoy["fecha"]
        fecha_manana = fila_manana["fecha"]

        qqq_close_hoy = fila_hoy.get("qqq_close", pd.NA)
        qqq3_close_hoy = fila_hoy.get("qqq3_close", pd.NA)
        qqq3_open_manana = fila_manana.get("qqq3_open", pd.NA)

        senal_confirmada_hoy = bool(fila_hoy.get("senal_confirmada", False))
        senal_base_on_hoy = bool(fila_hoy.get("senal_base_on", False))

        if pd.isna(qqq_close_hoy) or pd.isna(qqq3_close_hoy) or pd.isna(qqq3_open_manana):
            continue
        ...
        # ======================================================
        # 1) Ejecutar órdenes pendientes al open de mañana
        # ======================================================
        ...
        if entrada_pendiente and operacion_abierta is None:
            ...
            if qqq3_open_manana > 0:
                unidades = int(math.floor(presupuesto / qqq3_open_manana))
        ...
        # ======================================================
        # 2) Con la barra de HOY, decidir qué hacer mañana
        # ======================================================

        if operacion_abierta is None:
            if senal_confirmada_hoy:
